...
3.5.5 与旧代码的接口
   1.不能用数组初始化另一个数组，也不允许用vector初始化数组，但是可以用数组初始化vector
   即：指明要拷贝区域的首尾地址就可以了。也可以用数组的一部分初始化vector。
   int arr[]={1,2,3,4,5};
   vector<int> vec(begin(arr),end(arr));
   2.建议：应该尽量使用标准库类型而不是用数组。因为数组以及指针很容易出错，现代的C++
   程序应该尽量使用vector和迭代器，避免数组和指针的使用；应该尽量用string，避免使用C
   风格的基于数组的字符串。
3.6 多维数组
   1.使用范围for语句处理多维数组：
   int arr[3][4];
   ...
   注意到这里的代码for循环内部使用的都是引用类型，不只是因为这里需要修改数组的值，事实
   上，要使用范围for语句处理多维数组，那么除了最内层的循环，其他层的循环应该都是引用类
   型。如：
   for(auto &i:arr)
   {
     for(auto j:i)
