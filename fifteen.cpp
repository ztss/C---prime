...
15.7.3 派生类的拷贝控制成员
   ...
   2.在默认情况下，基类默认构造函数初始化派生类对象的基类部分。如果我们想拷贝或移动基类
   部分，则必须在派生类的构造函数初始值列表中显式的使用基类的拷贝或移动构造函数。
   3.如果构造函数或者析构函数调用了某个虚函数，则我们应该执行与构造函数或析构函数所属类
   型相对应的虚函数版本。
15.7.4 继承的构造函数
15.8 容器与继承
   1.当派生类对象被赋值给基类对象时，其中的派生类部分将被切掉。因此容器和存在继承关系的
   类型无法兼容。
   2.当我们希望在容器中存放具有继承关系的对象时，我们实际上存放的是基类的指针，而且最好
   是智能指针。
15.8.1 编写Basket类
15.9 文本查询程序再探
