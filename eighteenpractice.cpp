...
18.3.1 多重继承
   1.class panda : public Bear,public Endangered{};
   2.构造一个派生类的对象将同时构造并且初始化它的所有基类子对象。
18.3.2 类型转化与多个基类
   1.只有一个基类的情况下，派生类的指针或者引用能自动转化成一个可以访问基类的指针或者引
   用。多个基类的情况下，我们可以让某个可访问基类的指针或者引用直接指向一个派生类对象。
   2.与只有一个基类的继承一样，对象，指针和引用的静态类型决定我们能够使用那些成员。
18.3.3 多重继承下的类作用域
   1.当一个类拥有多个基类的时候，有可能出现派生类从两个或者更多基类中继承了同名成员的情
   况，此时，不加前缀限定符直接使用该名字将引发二义性。而最好的办法是在派生类也定义一个
   这个成员函数解决二义性问题。
18.3.4 虚继承
   1.虚继承的目的是令某个类做出声明，承诺愿意共享它的基类，其中，共享的基类子对象称为虚
   基类。
   2.虚派生只影响从指定了虚基类的派生类中进一步派生出的类，它不会影响派生类本身。
18.3.5 构造函数与虚继承
