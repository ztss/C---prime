...
13.5 动态内存管理
   ...
   2.使用移动构造函数，即move函数，需要utility头文件。
13.6 对象移动
   1.存在IO类和unique_ptr这样的类，这些类包含不能被共享的资源。因此，这些类型的对象不能
   拷贝但是可以移动。移动对象的特性只存在与C++的新特性中。
13.6.1 右值引用
   1.为了支持移动操作，新标准引入了一种新的引用类型，即右值引用。可以使用&&来获得右值引
   用，右值引用只能绑定到一个将要销毁的对象。
   2.右值引用只能绑定到临时对象，我们得知所引用的对象将要被销毁，该对象没有其他用户。
   3.我们可以使用move函数，这个函数定义在utility头文件中。这个函数告诉编译器，我们有一
   个左值。但是我们希望像一个右值一样处理它。使用move必须使用std::move。
13.6.2 移动构造函数和移动赋值函数
   1.我们可以定义我们自己的移动构造函数。如下：
   StrVec::StrVec(StrVec &&s) noexcept : element(s.element),
   first_free(s.first_free) ,cap(s.cap){
     s.element=s.first_free=s.cap=nullptr;
   }
   移动构造函数不应该抛出任何异常，所以加上noexcept。与拷贝构造函数不同，移动构造函数不
   分配任何内存，它只是接管给定的strVEc中的内存。在接管内存后，它将给定对象都置为nullptr
   。这样完成了给定对象的移动操作，此对象继续存在。最后，移动源对象被销毁，即指向析构函
   数。
   2.移动赋值运算符如下：
   ...
   3.只有当一个类没有定义任何自己版本的拷贝控制函数，并且它的所有数据成员都能移动构造或
   者赋值的时候，编译器才会为它合成移动构造函数或者移动赋值函数。
   4.定义了移动构造函数或者移动赋值运算符的类也必须定义自己的拷贝操作，否则，这些成员是
   默认删除的。
   5.所有的五个拷贝控制成员应该看成一个整体，一般来说，定义了一个拷贝操作，那么就应该定
   义所以的五个操作。
   6.在移动构造和移动赋值运算符这些类实现代码之外，只有当确定需要使用移动操作并且是安全
   的，才可以使用std::move。
